...
class Graph:
    def __init__(self, mode="train"):
        ...
        if mode=='infer':
            self.A_x = tf.placeholder(tf.float32, shape=(1, hp.fix_seq_length, 1+hp.n_fft//2))
            self.B_x = tf.placeholder(tf.float32, shape=(1, hp.fix_seq_length, 1+hp.n_fft//2))
        else:
            self.A_x = tf.placeholder(tf.float32, shape=(hp.batch_size, hp.fix_seq_length, 1+hp.n_fft//2))
            self.B_x = tf.placeholder(tf.float32, shape=(hp.batch_size, hp.fix_seq_length, 1+hp.n_fft//2))
        ...
        # monitor
        self.audio_A = tf.py_func(spectrogram2wav, [self.A_x[0]], tf.float32)
        self.audio_B = tf.py_func(spectrogram2wav, [self.B_x[0]], tf.float32)
        self.audio_A_to_B = tf.py_func(spectrogram2wav, [self.B_y_hat[0]], tf.float32)
        self.audio_B_to_A = tf.py_func(spectrogram2wav, [self.A_y_hat[0]], tf.float32)
        self.audio_A_to_B_to_A = tf.py_func(spectrogram2wav, [self.A_cycle_y_hat[0]], tf.float32)
        self.audio_B_to_A_to_B = tf.py_func(spectrogram2wav, [self.B_cycle_y_hat[0]], tf.float32)
        self.audio_A_to_A = tf.py_func(spectrogram2wav, [self.A_identity_y_hat[0]], tf.float32)
        self.audio_B_to_B = tf.py_func(spectrogram2wav, [self.B_identity_y_hat[0]], tf.float32)
